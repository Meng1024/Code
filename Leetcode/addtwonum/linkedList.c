...
*/
struct DLink *_newDLink(TYPE v, struct DLink *nxt, struct DLink *pre) {
    struct DLink *ret = (struct DLink *) malloc(sizeof(struct DLink));
    ret->value = v;
    ret->next = nxt;
    ret->prev = pre;
    return ret;
...
void _initList(struct linkedList *lst) {
    ...
    assert(lst != NULL);
    lst->firstLink = _newEmptyDLink();
    lst->lastLink = _newEmptyDLink();
...
void _addLinkBefore(struct linkedList *lst, struct DLink *l, TYPE v) {
    ...
    assert(lst != NULL);
    assert(l != NULL);
    struct DLink *tmp = _newDLink(v, l, l->prev);
    tmp->next->prev = tmp;
    tmp->prev->next = tmp;
    lst->size++;
}
...
void freeLinkedList(struct linkedList *lst) {
    while (!isEmptyList(lst)) { /* remove the link right after the first sentinel */
...
void _printList(struct linkedList *lst) {
    ...
    struct DLink *p;
    assert(lst != NULL);
    p = lst->firstLink->next;
    while (p != lst->lastLink) {
        printf("%d\n", p->value);
        p = p->next;
    }
    puts("");
...
int containsList(struct linkedList *lst, TYPE e) {
    ...
    assert(lst != NULL);
    assert(!isEmptyList(lst));
    for (p = lst->firstLink->next; p != lst->lastLink; p = p->next) {
        if (EQ(p->value, e))
            return 1;
    }
    return 0;
